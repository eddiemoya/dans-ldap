...
//Usermeta ldap mappings object
require_once 'Simple-LDAP-Login-User-Mappings.php';
$user_map = new LDAP_User_Mappings;


/*echo '<pre>';
var_dump($user_map->wp_user_meta);
echo '</pre>';*/
...
//User LDAP Mappings
elseif ($_POST['stage'] == 'user-mappings') {
	
	$user_map->set_ldap_mappings($_POST)->save();
}
...
<select name="create_type">
	<?php foreach($role_names as $role=>$name):?>
		<option value="<?php echo $name;?>" <?php if($simpleldap_account_type == $name){echo 'selected="selected"';}?>><?php echo $name;?></option>
	<?php endforeach;?>
</select>
</p>
<p>
...
<div class="information_pane">
	<form style="display:inline;" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>&updated=true">
		<table>
		<tr><td colspan="3">* For Alternate DN field, enter as "DN; filter", where filter is an attribute name from the Base DN
		<tr><th>User Meta Field</th><th>Alternate DN<br>(Leave blank if using the base DN)</th><th>LDAP Attribute</th></tr>
		<?php foreach($user_map->ldap_mappings as $metakey=>$attrs): ?>
			<tr><td class="user-meta-name"><?php echo $metakey;?></td><td><input type="text" name="<?php echo LDAP_User_Mappings::ALT_DN_PREFIX . $metakey; ?>" value="<?php echo $attrs['alt_dn'];?>" /></td><td><input type="text" name="<?php echo LDAP_User_Mappings::LDAP_ATTR_PREFIX . $metakey; ?>" value="<?php echo $attrs['ldap_attr'];?>" /></td></tr>
		<?php endforeach; ?>
		<tr><td colspan="3">
			<input type="hidden" name="stage" value="user-mappings" />
			<input type="submit" name="submit" value="<?php _e('Update LDAP User Settings', 'simple-ldap-login')?>" />
		</td></tr>
		</table>
	</form>
</div>
